...
const METODOS_PAGAMENTO = [
  {
    ...
    icone: "💳",
    descricao: "Crédito à vista ou parcelado",
  },
  {
    valor: "CartaoDebito",
  ...
  },
  {
    ...
    icone: "🤝",
    descricao: "Pague ao garçom ou entregador",
  },
];
...
const CheckoutModal = ({ visivel, onFechar }) => {
  ...
  const CODIGO_PIX =
    "00020126580014BR.GOV.BCB.PIX0136sol-do-cerrado@restaurante.com.br5204000053039865802BR5925Sol do Cerrado Restaurante6009SAO PAULO62140510pagamento63041D3E";
  ...
  // PASSO 1: Adicionar estados para endereços
  ...
  // CORREÇÃO 1: Carregar endereços quando o modal abre
  useEffect(() => {
    const carregarEnderecos = async () => {
      if (!visivel || !usuario) return;

      try {
        const res = await apiFetch(`/Endereco/usuario/${usuario.id}`);
        if (res.ok) {
  ...
  const renderPainelPagamento = () => {
    if (
      ...
      metodoPagamento === "CartaoDebito"
    ) {
      return (
        <div className="checkout-painel-pagamento">
          <p className="painel-titulo">
            💳 Dados do{" "}
            {metodoPagamento === "CartaoCredito" ? "Crédito" : "Débito"}
          </p>
        ...
        <div className="checkout-painel-pagamento painel-pix">
          ...
          <div className="pix-qrcode">
            ...
            >
              ...
              />
              {/* Padrão interno */}
              {[20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80].map(
                (x, i) =>
          ...
          <p className="pix-validade">
            ⏱️ Código válido por <strong>15 minutos</strong>
          </p>
        </div>
      );
    }
      ...
      return (
        <div className="checkout-painel-pagamento painel-entrega">
          ...
          <p className="entrega-msg">
            Você pagará <strong>{formatCurrency(totalFinal)}</strong>{" "}
            diretamente ao
            {tipoAtendimento === "Presencial" ? " garçom" : " entregador"} no
            momento do
      ...
      <div className="checkout-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {etapa < 3 && (
          <div className="checkout-header">
            <div className="checkout-steps">
              <span className={etapa === 1 ? "step ativo" : "step concluido"}>
                1 Entrega
              </span>
            ...
            <button className="checkout-fechar" onClick={onFechar}>
              ✕
            </button>
          </div>
        )}
        ...
        {etapa === 1 && (
          <div className="checkout-corpo">
            ...
            {/* Seleção de endereço para delivery */}
            {(tipoAtendimento === "DeliveryProprio" ||
              tipoAtendimento === "DeliveryApp") && (
              <div className="checkout-secao">
                ...
                {enderecos.length === 0 ? (
                  <p className="checkout-aviso">
                    ...
                    <br />
                    <a href="/minha-conta">Ir para Minha Conta</a>
                  </p>
                ) : (
                  <div className="checkout-enderecos-lista">
                    {enderecos.map((endereco) => (
                      <div
                        key={endereco.id}
                        className={`checkout-endereco-card ${enderecoSelecionado?.id === endereco.id ? "checkout-endereco-selecionado" : ""}`}
                        onClick={() => {
                          console.log("Endereço clicado:", endereco.id);
                      ...
                      >
                        <input
                          ...
                          onChange={() => {}} /* Evento controlado pelo onClick do card */
                          style={{ cursor: "pointer" }}
                        />
                        <div className="checkout-endereco-info">
                          <p className="checkout-endereco-principal">
                            {endereco.rua}, {endereco.numero}
                          </p>
                          {endereco.complemento && (
                            <p className="checkout-endereco-complemento">
                              {endereco.complemento}
                            </p>
                          )}
                          <p className="checkout-endereco-local">
                            {endereco.bairro} — {endereco.cidade}/
                            {endereco.estado}
                          </p>
                          <p className="checkout-endereco-cep">
                            CEP: {endereco.cep}
                          </p>
                        </div>
                      </div>
            ...
            <button
              ...
              onClick={() => {
                // PASSO 3: Validar endereço para delivery
                if (tipoAtendimento !== "Presencial" && !enderecoSelecionado) {
              ...
              <div className="checkout-linha taxa">
                <span>
                  ...
                  {tipoAtendimento === "DeliveryProprio" &&
                    "🛵 Taxa de entrega"}
                  {tipoAtendimento === "DeliveryApp" &&
                    `📱 Taxa app (${new Date().getHours() < 18 ? "4%" : "6%"})`}
                </span>
                <span>{taxa === 0 ? "Grátis" : formatCurrency(taxa)}</span>
              </div>
              <div className="checkout-linha total">
